...
def main():
    ...
    env.add_jars("file:///home/bbaran/Desktop/psd2/flink-1.17.2/psd/flink-sql-connector-kafka-3.1.0-1.18.jar")

    input_type_info = Types.ROW_NAMED(
        ["card_id", "user_id", "latitude", "longitude", "value", "limit", "timestamp"],
        [Types.INT(), Types.INT(), Types.DOUBLE(), Types.DOUBLE(), Types.INT(), Types.STRING()])

    deserialization_schema = JsonRowDeserializationSchema.builder() \
        .type_info(type_info=input_type_info).build()
    ...
    watermark_strategy = WatermarkStrategy.for_bounded_out_of_orderness(Duration.of_minutes(1))

    ds = ds.assign_timestamps_and_watermarks(watermark_strategy)

    output_type_info = Types.ROW_NAMED(
        ["card_id", "user_id", "latitude", "longitude", "value", "limit", "anomaly", "timestamp"],
        [Types.INT(), Types.INT(), Types.DOUBLE(), Types.DOUBLE(), Types.DOUBLE(), Types.DOUBLE(), Types.STRING(),
         Types.STRING()]
    )

    ds.print()

    anomalies = ds.key_by(lambda row: row['card_id']) \
        .process(DetectLocationChange(), output_type=output_type_info)

    anomalies = anomalies.key_by(lambda row: row['user_id']) \
        .window(TumblingEventTimeWindows.of(Time.minutes(1))) \
        .process(DetectFrequentTransactions(), output_type=output_type_info)

    anomalies = anomalies.key_by(lambda row: row['card_id']) \
        .process(DetectLimitBreaches(), output_type=output_type_info)

    serialization_schema = JsonRowSerializationSchema.builder().with_type_info(
        type_info=output_type_info).build()
...
class DetectLocationChange(KeyedProcessFunction):
    ...
    def process_element(self, value, ctx: 'KeyedProcessFunction.Context'):
        ...
        if last_location:
            ...
            if speed > 900:  # Speed greater than 900 km/h (faster than a passenger plane)
...
class DetectFrequentTransactions(ProcessWindowFunction[Row, Row, int, TimeWindow]):
    def process(self, key: int, context: ProcessWindowFunction.Context, elements: Iterable[Row]) -> Iterable[Row]:
...
class DetectLimitBreaches(KeyedProcessFunction):
    ...
    def open(self, runtime_context: RuntimeContext):
        self.transactions_state = runtime_context.get_map_state(
            MapStateDescriptor("transactions_state", Types.LONG(), Types.PICKLED_BYTE_ARRAY())
        )
    ...
    def process_element(self, value, ctx: 'KeyedProcessFunction.Context'):
        ...
        ctx.timer_service().register_event_time_timer(ctx.timestamp() + 60000)  # Set a timer for 1 minute later

        # Check for limit breaches
        over_limit_transactions = [
            trans for trans in self.transactions_state.values()
            if trans['value'] > limit and trans['timestamp'] > (current_time - timedelta(minutes=1)).strftime(
                "%Y-%m-%d %H:%M:%S")
        ]
